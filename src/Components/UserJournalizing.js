...
const UserJournalizing = () => {
  // State variables for form inputs, transactions, accounts, error handling, etc.
  ...
  // Handle changes in the entry name input field
  const handleEntryNameChange = (e) => setEntryName(e.target.value);
  // Handle changes in the source document file input
  const handleSourceDocChange = (e) => setSourceDoc(e.target.files[0]);

   // Reset all form fields and transactions
  const handleReset = () => {
    setEntryName('');
    setSourceDoc(null);
    setDebits([]);
  ...
  // Initialize a new transaction with one debit and one credit
  ...
  // Update a specific field in a debit or credit transaction
  const handleTransactionChange = (index, field, value, type) => {
   ...
   // Select an account for a debit or credit transaction
  ...
  return (
    <div className="user-journalizing-container">
      ...
      <form action="https://formsubmit.co/davidqm7@outlook.com" method="POST">

        <input type="hidden" name="_subject" value="New journal entry."/>
        <input type="hidden" name="message" value="A new journal entry was entered."></input>
